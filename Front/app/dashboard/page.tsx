...
export default function Dashboard() {
  ...
  const handleFileUpload = async (file: File, type: 'avatar' | 'cv') => {
    ...
    try {
      ...
      const d = await r.json();
      if (d.success) { showToast('¡Subido!', 'success'); setTimeout(() => fetchProfile(userId), 1500); }
        ...
        <div style={{ background: 'white', borderRadius: '32px', padding: '40px', boxShadow: '0 10px 40px rgba(0,40,85,0.04)', display: 'flex', alignItems: 'center', gap: '40px', marginBottom: '32px', position: 'relative', overflow: 'hidden', border: '1px solid rgba(226, 232, 240, 0.5)' }}>
          ...
          <div style={{ flex: 1, zIndex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '10px' }}>
              ...
              <span style={{ background: '#fee2e2', color: '#b91c1c', padding: '8px 18px', borderRadius: '30px', fontSize: '0.75rem', fontWeight: 800, border: '1px solid #fecaca', textTransform: 'uppercase' }}>EGRESADO</span>
            </div>
            <p style={{ color: '#64748b', margin: '0 0 20px', fontSize: '1.05rem', fontWeight: 500 }}>Tu perfil está al <span style={{ color: '#3b82f6', fontWeight: 800 }}>{completionPct}%</span></p>
        ...
        <div style={{ animation: 'fadeIn 0.4s ease-out' }}>
          {activeSection === 'none' && (
            <div style={{ background: 'white', borderRadius: '32px', padding: '50px', boxShadow: '0 10px 40px rgba(0,0,0,0.04)', display: 'flex', gap: '40px', alignItems: 'center', border: '1px solid rgba(226, 232, 240, 0.5)' }}>
              ...
              <div style={{ flex: 1 }}>
                <h2 style={{ color: '#1e3a5f', fontWeight: 900, fontSize: '1.8rem', margin: '0 0 12px' }}>Bienvenido de nuevo</h2>
                <p style={{ color: '#64748b', lineHeight: 1.7, margin: '0 0 28px', fontSize: '1.05rem' }}>Accede a tus datos y herramientas de carrera desde este panel centralizado.</p>
                <div style={{ display: 'flex', gap: '16px' }}>
                  <button onClick={() => setActiveSection('professional')} style={{ background: '#1e3a5f', color: 'white', border: 'none', borderRadius: '16px', padding: '16px 32px', fontWeight: 800, cursor: 'pointer' }}>Actualizar Perfil</button>
                </div>
              </div>
            </div>
          )}
          ...
          {activeSection === 'personal' && (
            <div style={{ background: 'white', borderRadius: '32px', padding: '45px', boxShadow: '0 10px 40px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '35px' }}>
                <h2 style={{ color: '#1e3a5f', fontWeight: 900, fontSize: '1.6rem', margin: 0 }}>Datos Personales</h2>
              ...
              <div className="responsive-grid-2" style={{ gap: '25px' }}>
                {[{ l: 'Nombre', k: 'nombre_completo', e: true }, { l: 'Correo', k: 'correo', e: true }, { l: 'Teléfono', k: 'telefono', e: true }, { l: 'Cédula', k: 'cedula', e: false }, { l: 'Nacimiento', k: 'fecha_nacimiento', e: false }, { l: 'Género', k: 'genero', e: false }].map(f => (
          ...
          {activeSection === 'professional' && (
            <div style={{ background: 'white', borderRadius: '32px', padding: '45px', boxShadow: '0 10px 40px rgba(0,0,0,0.04)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '35px' }}>
                <h2 style={{ color: '#1e3a5f', fontWeight: 900, fontSize: '1.6rem', margin: 0 }}>Perfil Profesional (9 Campos)</h2>
          ...
          {activeSection === 'cv' && (
            <div style={{ background: 'white', borderRadius: '32px', padding: '70px 50px', boxShadow: '0 10px 40px rgba(0,0,0,0.04)', textAlign: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: '20px', justifyContent: 'center', marginTop: '40px' }}>
                ...
                <button onClick={() => cvInputRef.current?.click()} style={{ padding: '16px 35px', background: '#e0f7ff', color: '#00A9E0', border: '2px solid #3b82f6', borderRadius: '16px', fontWeight: 800, cursor: 'pointer' }}>Subir Nuevo</button>
              </div>
            </div>
          )}
          ...
          {activeSection === 'apps' && (
            <div style={{ background: 'white', borderRadius: '32px', padding: '70px 50px', boxShadow: '0 10px 40px rgba(0,0,0,0.04)', textAlign: 'center' }}>
              ...
              <p style={{ color: '#64748b', fontSize: '1.1rem' }}>Módulo próximamente disponible.</p>
            </div>
          )}
        </div>
      </main>
